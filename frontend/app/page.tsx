...
import { ArrowDownWideNarrow, ArrowRight, Bookmark, BriefcaseBusiness, Check, ChevronRight, CircleHelp, Compass, FileCheck2, GraduationCap, LayoutGrid, LoaderCircle, Plus, Search, ShieldCheck, SlidersHorizontal, Sparkles, Target, TrendingUp, X } from "lucide-react";
...
type View = "catalog" | "mine" | "saved" | "proposals";
...
export default function Home() {
  ...
  const [proposalEpoch, setProposalEpoch] = useState(0);
  ...
  useEffect(() => {
    let cancelled = false;
    const fetchTasks = view === "saved" && profileId ? api.saved(profileId) : api.listTasks((view === "mine" || view === "proposals") && role === "business");
  ...
  useEffect(() => {
    if (!editorDirty && !proposalDirty && !detailDirty && !proposalBusy && !detailBusy) return;
    const warn = (event: BeforeUnloadEvent) => { event.preventDefault(); event.returnValue = ""; };
    window.addEventListener("beforeunload", warn);
    return () => window.removeEventListener("beforeunload", warn);
  }, [editorDirty, proposalDirty, detailDirty, proposalBusy, detailBusy]);

  function leaveWorkspace(action: () => void) {
    if (proposalBusy || detailBusy || busy) { toast.info("Дождитесь завершения сохранения."); return; }
  ...
  function closeDetail() { setDetail(null); setDetailDirty(false); setDetailBusy(false); }
  function openTask(id: string) {
    leaveWorkspace(() => {
      api.getTask(id, role === "business").then((task) => { setDetailDirty(false); setDetail(task); }).catch((err) => toast.error(err.message));
    });
  }
  ...
  const viewTitle = view === "proposals" ? (role === "business" ? "Отклики команд" : "Мои отклики") : view === "mine" ? "Мои задачи" : view === "saved" ? "Мои квесты" : "Каталог задач";
  ...
  return <SidebarProvider style={{ "--sidebar-width": "242px", "--sidebar-width-mobile": "260px" } as React.CSSProperties}>
    <Sidebar className="alem-sidebar">
      ...
      <SidebarContent className="sidebar-body"><p className="nav-caption">РАБОЧЕЕ ПРОСТРАНСТВО</p><SidebarMenu>
        ...
        <SidebarMenuItem><WorkspaceNavButton isActive={view === "proposals" && !editor} onClick={() => navigate("proposals")}><FileCheck2/><span>{role === "business" ? "Отклики команд" : "Мои отклики"}</span></WorkspaceNavButton></SidebarMenuItem>
