...
import { ArrowUpRight, Check, CheckCheck, Clock3, Download, ExternalLink, FileText, Loader2, RotateCcw, Save, X } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/textarea";
import { api } from "@/lib/api";
import type { Proposal, ProposalAttachment, ProposalStatus, Role } from "@/lib/contracts";
import ProjectCompletionDialog from "@/components/rewards/ProjectCompletionDialog";
import completionStyles from "@/components/rewards/completion.module.css";
...
export default function ProposalCard({ proposal, role, onChanged, onOpenTask, onDirtyChange, onBusyChange }: Props) {
  ...
  const commentDirty = role === "business" && !proposal.completion && comment !== proposal.decisionComment;
  const dirty = commentDirty || completionDirty;
  ...
  return <article className={`alem-proposal-card alem-proposal-${proposal.status}`}>
    <header className="alem-proposal-card-header"><div><span className="alem-proposal-kicker">Предложение команды</span><h3>{proposal.team.name}</h3><small>{formatDate(proposal.createdAt)}</small></div><span className={`alem-proposal-status alem-proposal-status-${proposal.status}`}>{proposal.completion ? "Проект завершён" : PROPOSAL_STATUSES[proposal.status]}</span></header>
    ...
    {proposal.completion && <section className={completionStyles.completed} aria-label="Подтверждённый результат проекта"><div className={completionStyles.completedHeading}><CheckCheck size={18} /><strong>Проект завершён</strong><span>+{proposal.completion.pointsAwarded} баллов</span></div><p>{proposal.completion.summary}</p><small>Бизнес подтвердил результат {formatDate(proposal.completion.confirmedAt)}</small></section>}
    {role === "business" && !proposal.completion ? <div className="alem-proposal-decision">
      <div className="alem-proposal-field"><label htmlFor={commentId}>Комментарий команде <span>необязательно</span></label><Textarea id={commentId} value={comment} onChange={(event) => setComment(event.target.value)} placeholder="Обратная связь или следующие шаги" maxLength={2000} rows={3} disabled={!!busy || completionBusy} /></div>
      {error && <p className="alem-proposal-error" role="alert">{error}</p>}
      <div className="alem-proposal-decision-buttons">
        {proposal.status !== "accepted" && <Button type="button" className="ha-task-primary" disabled={!!busy || completionBusy} onClick={() => void decide("accepted")}>{busy === "accepted" ? <Loader2 size={15} className="ha-task-spin" /> : <Check size={15} />}Выбрать команду</Button>}
        {proposal.status !== "rejected" && <Button type="button" variant="outline" disabled={!!busy || completionBusy} onClick={() => void decide("rejected")}>{busy === "rejected" ? <Loader2 size={15} className="ha-task-spin" /> : <X size={15} />}Отклонить</Button>}
        {proposal.status !== "submitted" && <Button type="button" variant="outline" disabled={!!busy || completionBusy} onClick={() => void decide("submitted")}>{busy === "submitted" ? <Loader2 size={15} className="ha-task-spin" /> : <RotateCcw size={15} />}На рассмотрение</Button>}
        {commentDirty && <><Button type="button" variant="outline" disabled={!!busy || completionBusy} onClick={() => void decide(proposal.status)}><Save size={15} />Сохранить комментарий</Button><Button type="button" variant="ghost" disabled={!!busy || completionBusy} onClick={() => setComment(proposal.decisionComment)}>Сбросить комментарий</Button></>}
      </div>
      {proposal.status === "accepted" && <div className={completionStyles.completionAction}><ProjectCompletionDialog proposal={proposal} disabled={!!busy || !!commentDirty} onChanged={onChanged} onDirtyChange={setCompletionDirty} onBusyChange={(value) => { setCompletionBusy(value); onBusyChange(value); }} /><p>{commentDirty ? "Сохраните или сбросьте комментарий перед подтверждением завершения." : "После проверки результата вы можете начислить команде 100 баллов за успешный проект."}</p></div>}
    </div> : proposal.decisionComment ? <div className="alem-proposal-feedback"><strong>Комментарий бизнеса</strong><p>{proposal.decisionComment}</p></div> : !proposal.completion ? <p className="alem-proposal-hint">{proposal.status === "submitted" ? "Предложение ожидает решения бизнеса." : proposal.status === "accepted" ? "Бизнес выбрал вашу команду. Следующие шаги можно уточнить по контакту в задаче." : "Бизнес отклонил это предложение. Можно подготовить новый отклик."}</p> : null}
  </article>;
}
