...
**Работает сценарий до ручного выбора команды.** Доступны редактор и публикация карточек, рейтинг с автоматической проверкой содержания, открытый каталог, тестовые профили, квесты, отправка откликов с PDF, сравнение предложений и решения бизнеса. AI-конструктор возвращает вопросы и карточку с источниками для проверки человеком; без ключа внешнего API используется обозначенная локальная заглушка. Сохранение квеста и отправка отклика — разные действия; команда выбирается только явным решением бизнеса.
...
Последний полный прогон реализации от **23 сентября 2026 года**: **132 backend-теста пройдены** (58 прежних проверок, 33 проверки откликов, 27 качества, 13 PDF и 1 сквозной сценарий), `pip check`, TypeScript, ESLint и production-сборка успешны. В браузере проверены отправка PDF, скачивание, ручной выбор бизнеса, статус у команды, оценка критериев и мобильный вид. Через Next.js проверена загрузка PDF ровно 10 МиБ и отказ при превышении. Есть одно предупреждение Starlette об устаревающем `httpx` в TestClient. Реальный вызов внешней модели этим прогоном не проверялся. Подробности и границы проверки — [docs/STAGE2_VERIFICATION.md](docs/STAGE2_VERIFICATION.md).
...
Подробности: [отклики и PDF](docs/PROPOSALS.md), [проверка качества](docs/QUALITY.md), [актуальный отчёт о проверках](docs/STAGE2_VERIFICATION.md), [исторический контракт первой части](docs/PART1_CONTRACT.md), [план полного MVP](DEVELOPMENT_PLAN.md).
